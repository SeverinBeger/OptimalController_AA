...
function [CD,M] = DragCoefficient(h_met,v_met,l,l_b,l_nc,d,d_b,S_B,l_rootchord, l_tipchord,S_F,X_tc,t_fins,nf,A_AAsmall,A_AAbig,n_AA,phi)
%Inputs:
...
%n_f - number of fins
%A_AAsmall - Active Aero Fin small area (thickness times span)
%A_AAbig - Active Aero Fin main area ((rootchord+tipchord)/2 * span)
%n_AA - amount of AA fins used
%phi - current angle of the fins
...
X_tc = X_tc/in2m;
t_fins = t_fins/in2m;
A_AAsmall = A_AAsmall / (in2m^2);
A_AAbig = A_AAbig / (in2m^2);
...
%     M = 143.4*0.0008957066031914082  %Ideal mach number
 ...
 %Body Tube
    ...
    CD_f_body = Cf_final * (1+ 60/((l/d)^3) + 0.0025 * (l/d)) * S_B/A_r;
 ...
 %Fins
   ...
   if(Cf_lambda<0 || isnan(Cf_lambda))
       Cf_lambda = 0;
   end
       Cf_lambda = real(Cf_lambda); 
   %Body Coefficient of drag due to skin fricion
...
%% Active Aero Fins Drag
    %mainly pressure drag. Calculate with artificial function. Should be
    %CFD Data or wind tunnel measurements for accuracy. Here just proof of
    %concept
    CD_AA_raw = CD_AAcalc(phi);
    A_AA_perp = A_AAbig * sin(phi) + A_AAsmall * cos(phi); %perpendicular area of the fin
    CD_AA = CD_AA_raw * (n_AA * A_AA_perp)/A_r;
    
%% Total Drag Coefficient

CD = CD_f + CD_b + Del_CD_T + Del_CD_S + K_F * CD_AA;
end
